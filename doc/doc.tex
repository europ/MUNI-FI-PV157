...
\section*{Multiple choice}
    ...
    \begin{minipage}{\textwidth}
        \textbf{ \mypara Úspěšnost hádání hesel hrubou silou:}\\[-7.5mm]
        \begin{enumerate}[label={(\alph*)}]
            \item klesá s dobou platnosti hesla\\[-7.5mm]
            \item roste s velikostí použité abecedy\\[-7.5mm]
            \item ovlivnil výzkum Needhama a Schroedera (2008)\\[-7.5mm]
            \item lze u online útoků omezit procedurálně\\[-7.5mm]
            \item klesá s délkou hesla\\[-7.5mm]
            \item roste s rostoucí rychlostí útočníkova počítače\\[-7.5mm]
        \end{enumerate}
        \textit{\textbf{Správně}}: d, e, f\\[-3mm]
    \end{minipage}

\section*{Full text}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Napište 2 výhody a 2 nevýhody autentizace biometrikou oproti jiné metodě.}\\[-7.5mm]
        \begin{itemize}
            \item[\red{\textbf{+}}] Nemůžeme ztratit, zapomenout nebo předat jiné osobě\\[-7.5mm]
            \item[\red{\textbf{+}}] Rychlé a (relativně) přesné výsledky\\[-7.5mm]
            \item[\blue{\textbf{--}}] Uživatelé s poškozenými/chybějícími orgány mohou mít problém\\[-7.5mm]
            \item[\blue{\textbf{--}}] Mohou příliš vypovídat o uživatelích, citlivé informace\\
        \end{itemize}
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Popište, jak probíhá man in the middle útok na Diffie-Hellman protokol.}\\
        Mallory (aktivní útočník provádějící útok man in the middle) může navázat dvě různé klíčové výměny, jednu s Alice a druhou s Bobem, účinně předstírá, že je Alice, k Bobovi, a naopak, což jí umožní dešifrovat, a pak zašifrovat zprávy mezi nimi. Všimněte si, že Mallory musí zůstat ve středu, přenášet zprávy pokaždé, když Alice a Bob komunikují. Pokud to někdy neudělá, její předchozí přítomnost je pak odhalena Alici a Bobovi. Budou vědět, že všechny jejich soukromé zprávy někdo zachytil a dekódoval.\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Co jsou a jak fungují tokeny založené na hodinách? Jaké jsou jejich bezpečnostní nedostatky?}\\[2mm]
        Bývají součástí autentizačních kalkulátorů. V daném okamžiku dávají správnou hodnotu. Jedinečnou pro daný přístroj. Platnou pouze po určitou dobu. Tuto hodnotu umí spočítat i autentizační server. Bezpečnostním nedostatkem je ztráta synchronizace hodin, otázka platnosti časových rámců před a po.\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Model Bell-LaPadula.}\\[2mm]
        K zachování důvěrnosti citlivých dat. Procesy nesmějí zapisovat data do nižší úrovně. Procesy nesmějí číst data na vyšší úrovni.\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Statická autentizace dat u EMV.}\\[2mm]
        Potvrzuje pravost statických dat uložených v čipové kartě. Detekuje neautorizovanou změnu dat po personalizaci karty. Prováděna terminálem (čip pouze zasílá potřebná data).\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Výkonová analýza u čipových karet.}\\[2mm]
        Analyzujeme, jaké operace se provádějí na kartě při zadání určité sekvence. Pokud například při špatném PINu se jen sníží čítač a u správného PINu ne, tak poznáme lehce, kdy jsme zadali správný a kdy ne. Tomuto se musí předcházet tím, že oba tyto běhy nastavíme tak, aby nebyly odlišné a útočník analýzou nezjistil nové informace.\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Časové razítka.}\\[2mm]
        Časově proměnný parametr. Zajišťují jedinečnost a časovou přesnost. Obě strany musí synchronizovat a zabezpečit hodiny.\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Výhody a nevýhody autentizácie heslom.}\\[-7.5mm]
        \begin{itemize}
            \item[\red{\textbf{+}}] Snadno zapamatovatelné a složité heslo je pro uživatele snadnou autentizací, pro útočníka naopak velice náročné uhodnout\\[-7.5mm]
            \item[\red{\textbf{+}}] Nemusí tolik vypovídat o uživateli jako biometriky\\[-7.5mm]
            \item[\blue{\textbf{--}}] Heslo je možné odposlechnout při autentizaci\\[-7.5mm]
            \item[\blue{\textbf{--}}] Heslo můžeme zapomenout\\
        \end{itemize}
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Popište stručně jak se provádí odběrová analýza u čipové karty.}\\[2mm]
        Využívá se: Závislost průběhu odběru proudu na ukládaných datech do paměti EEPROM. Závislost průběhu odběru proudu na zpracovávaných datech. Závislost průběhu odběru proudu na prováděné instrukci. Dle toho můžeme zjistit, zda jsme zadali správné heslo nebo PIN.\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Jak probíhá Shamirův protokol.}\\[2mm]
        Nevyžaduje žádné ustavení sdílených klíčů. Vyžaduje komutativní šifrovací algoritmus. Každá strana má svůj klíč. Aby obě strany měli stejnou informaci (klíč), musí si vyměnit 3 zprávy.\\
    \end{minipage}

    \begin{minipage}{\textwidth}
        \textbf{ \mypara Popište navzájem klady a zápory hesel a tokenů}\\[2mm]
        Token\\
        \begin{itemize}
            \item[\red{\textbf{+}}] Rychlé zjištění ztráty\\[-7.5mm]
            \item[\red{\textbf{+}}] Mohou zpracovávat a přenášet další informace\\[-7.5mm]
            \item[\blue{\textbf{--}}] Bez tokenu není autorizovaný uživatel rozpoznán\\[-7.5mm]
            \item[\blue{\textbf{--}}] Ke kontrole je obvykle třeba speciální čtečka nebo vycvičená osoba\\
        \end{itemize}
        Heslo
        \begin{itemize}
            \item[\red{\textbf{+}}] Snadno zapamatovatelné a složité heslo je pro uživatele snadnou autentizací, pro útočníka naopak velice náročné uhodnout\\[-7.5mm]
    ...
    \begin{minipage}{\textwidth}
        \textbf{ \mypara Napište a popište alespoň 2 autentizační primitiva.}\\[2mm]
        Hash (spočítána pomocí hašovací funkce z nějaké informace, například heslo), Heslo (obecný prostředek k autentizaci uživatele), PIN (doplňkový bezpečnostní mechanismus pro operace s bankomatem).\\
    \end{minipage}
